...
.q-pa-md
    q-card.absolute-center.q-pa-sm(style="width: 40%")
      q-card
        .text-h5.text-center.login-title.q-my-md Editar Producto
        Form(:validation-schema='userSchema')
          ...
          div.flex.justify-center.q-mb-md
            q-btn(label="Editar Producto" color="black" square @click="validateUserForm()")
</template>
...
const editProduct = async () => {
  ...
  const response = await store.put('product/', store.product + '/', formData)
  response >= 200 && response < 300 ? store.notify('Exito!', 'Producto editado', 'success') : store.notify('Error', 'Error al editar el producto', 'error')
  cleanFields()
}
...
const get = async () => {
  const product = await store.getId('product/', store.product)
